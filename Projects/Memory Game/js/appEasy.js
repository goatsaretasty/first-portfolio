...
const cardNames = ['ninja', 'glasses', 'game', 'frog', 'eye', 'poo', 'dice', 'agent'];
...
function startGame() {
  ...
  cards = shuffle(cards);
    ...
    cards.forEach(function (card) {
      ...
      prefix so I don't have to list all of these "celebrate-" classes. TODO update 6/24/18: figured it out!
      See the literal notation on line 94 below! */
      ...
      for (i = 0; i < cardNames.length; i++) {
        card.classList.remove(`celebrate-${cardNames[i]}`);
      }
    });
...
exactly two cards and then runs the moveCounter() function. If the two cards' keys match
...
Otherwise, if the two cards' keys are not the same, the notAMatch() function is called,
...
function matchEval() {
  ...
  if (length === 2) {
    ...
    moveCounter();
    if (openedCards[0].getAttribute('data-key') === openedCards[1].getAttribute('data-key')) {
      isAMatch();
    ...
    } else if (openedCards[0].getAttribute('data-key') !== openedCards[1].getAttribute('data-key')) {
...
function isAMatch() {
  for (card of openedCards) {
    card.classListChain.add('match', 'match-animation')
     .classListChain.remove('show', 'open');
    openedCards = [];
  }
}
/*
...
function notAMatch() {
  ...
  setTimeout(function () {
    for (card of openedCards) {
      card.classList.remove('show', 'open', 'unmatched');
      enable();
      openedCards = [];
    }
  }, 600);
...
function gameOver() {
  if (matchedCard.length === 16) {
    ...
    */
    (moves >= 8 && moves <= 9) ?
    (totalMoves.innerHTML = `At ${moves} moves, ${ratingText.perfect}`) :
    ((moves > 9 && moves <= 12) ?
      (totalMoves.innerHTML = `At ${moves} moves, ${ratingText.great}`) :
      ((moves > 12 && moves <= 15) ?
        (totalMoves.innerHTML = `At ${moves} moves, ${ratingText.notHalfBad}`) :
        ((moves > 15 && moves < 18) ?
          (totalMoves.innerHTML = `At ${moves} moves, ${ratingText.halfBad}`) :
          ((moves > 18 && moves <= 21) ?
            (totalMoves.innerHTML = `At ${moves} moves, ${ratingText.notTerrible}`) :
            ((moves > 21 && moves <= 24) ?
              (totalMoves.innerHTML = `At ${moves} moves, ${ratingText.playAgain}`) :
              ((moves > 24) ?
              (totalMoves.innerHTML = `At ${moves} moves, ${ratingText.worst}`) :
              undefined))))));
...
const ratingText = {
  perfect: "you had a perfect score!",
  great: "you did great!",
  notHalfBad: "you weren't half bad!",
  halfBad: "you were exactly half good and half bad!",
  notTerrible: "you're not terrible!",
  playAgain: "you should play again!",
  worst: "do you even remember your own name?"
}
...
function matchAnimations() {
  cards.forEach(function(card) {
    for (i = 0; i < cardNames.length; i++) {
      (card.getAttribute('data-key') === cardNames[i]) ?
      (card.classListChain.remove('match-animation').classListChain.add(`celebrate-${cardNames[i]}`)) : undefined;
    }
